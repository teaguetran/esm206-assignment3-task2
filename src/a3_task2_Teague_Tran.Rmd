...
mpg_make_model <- mpg %>% # Start with mpg
  unite(col = make_model, # Create a new column called 'make_model'
        ...
        sep = " HEYA HEEEEEYYAAAAAAAAA " # Puts "HEY!" between the pieces (default is underscore)
        ) 
...
gasol_hookshots <- lakers %>% 
  ...
  mutate(month_shot = month(date_ymd))

gasolhook_0809 <- gasol_hookshots %>% 
  group_by(etype, month_shot) %>% 
  count(month_shot) 
